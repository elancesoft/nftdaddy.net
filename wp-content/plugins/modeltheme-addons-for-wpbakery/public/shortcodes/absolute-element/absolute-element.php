...
function modeltheme_addons_for_wpbakery_absolute_element( $params, $content ) {
    ...
    <?php if (array_key_exists('elements', $params)) { ?>
        <?php $elements = vc_param_group_parse_atts($params['elements']);?>
        <?php if ($elements) { ?>
            <?php foreach($elements as $element){ 
               ...
               if (!array_key_exists('element_photo_height', $element)) {
                ...
                <?php  if ($image_attributes) { ?>
                    <img class="mt-addons-absolute-element-absolute <?php echo $animation_type;?>"  style="left:<?php echo esc_attr($left_percent);?>%;top:<?php echo esc_attr($top_percent);?>%; <?php if($element_photo_height){echo 'height: '.$element_photo_height.'px;';} ?>" src="<?php echo esc_url($image_attributes[0]); ?>" alt="<?php echo esc_html__($absolute_element); ?>" />
                <?php } ?>
            <?php } ?>
        <?php } ?>
    <?php } ?>
