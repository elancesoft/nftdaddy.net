...
if (!function_exists('enefti_logo_source')) {
    function enefti_logo_source(){
        ...
        $html = '';
        // Metaboxes
        $mt_custom_header_options_status = get_post_meta( get_the_ID(), 'enefti_custom_header_options_status', true );
        $mt_metabox_header_logo = get_post_meta( get_the_ID(), 'enefti_metabox_header_logo', true );
        if (is_page()) {
            if (isset($mt_custom_header_options_status) && isset($mt_metabox_header_logo) && $mt_custom_header_options_status == 'yes') {
                if($mt_metabox_header_logo) {
                    ...
                    $html .='<img src="'.esc_url($enefti_redux['enefti_logo']['url']).'" alt="'.esc_attr(get_bloginfo()).'" />';
                }
            }else{
...
if (!function_exists('enefti_body_classes')) {
    function enefti_body_classes( $classes ) {
        ...
        if (is_page()) {
            ...
            $is_transparent = '';
            if (isset($mt_custom_header_options_status) AND $mt_header_custom_transparent == 'yes') {
                if ($mt_header_custom_transparent == true) {
                    // Header Layout #1
